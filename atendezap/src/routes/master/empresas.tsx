...
import { LogIn, Pause, Play, CalendarClock, Loader2, KeyRound, Eye, Pencil, Save } from "lucide-react";
import { brand } from "@/config/brand";
import { listCompanies, suspendCompany, extendTrial, resetCompanyOwnerPassword, getCompanyDetails, updateCompanyAndOwner } from "@/lib/master.functions";
...
function EmpresasPage() {
  ...
  const update = useServerFn(updateCompanyAndOwner);
  ...
  async function openEdit(c: any) {
    setEditing({ loading: true, companyId: c.id });
    try {
      const r: any = await details({ data: { companyId: c.id } });
      const owner = r.members?.find((m: any) => m.role === "owner") ?? r.members?.[0];
      setEditing({
        companyId: c.id,
        companyName: r.company?.nome ?? "",
        telefone: r.company?.telefone ?? "",
        ownerName: owner?.profile?.nome ?? "",
  ...
  return (
    <div className="space-y-4">
            ...
            {rows.map((c) => (
              <li key={c.id} className="grid grid-cols-12 px-4 py-3 items-center text-sm">
                ...
                <div className="col-span-2 flex justify-end gap-1">
                  <Button size="sm" variant="outline" onClick={() => openEdit(c)} title="Editar nome / e-mail / empresa">
      ...
      {/* Editar dados (repaginar conta demo) */}
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Editar empresa e responsável</DialogTitle></DialogHeader>
          {editing?.loading ? (
            ...
            <div className="space-y-4 py-1">
              <div className="space-y-1.5">
                ...
                <Input value={editing.companyName} onChange={(e) => setEditing({ ...editing, companyName: e.target.value })} placeholder="Ex.: Green Yards LLC" />
              </div>
              <div className="space-y-1.5">
                <Label>Nome do responsável</Label>
                <Input value={editing.ownerName} onChange={(e) => setEditing({ ...editing, ownerName: e.target.value })} placeholder="Ex.: John Smith" />
              </div>
              <div className="space-y-1.5">
                <Label>E-mail do responsável</Label>
                <Input type="email" value={editing.ownerEmail} onChange={(e) => setEditing({ ...editing, ownerEmail: e.target.value })} placeholder="cliente@empresa.com" />
                <p className="text-[11px] text-muted-foreground">Vira o login do cliente. Use o e-mail que o prospect te passou.</p>
              </div>
              <div className="space-y-1.5">
                <Label>Telefone</Label>
                <Input value={editing.telefone} onChange={(e) => setEditing({ ...editing, telefone: e.target.value })} placeholder="+1 (407) 555-0100" />
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setEditing(null)}>Cancelar</Button>
                <Button onClick={saveEdit} disabled={savingEdit}>
                  {savingEdit ? <Loader2 className="size-4 mr-1.5 animate-spin" /> : <Save className="size-4 mr-1.5" />} Salvar
                </Button>
              </DialogFooter>
            </div>
          ) : null}
