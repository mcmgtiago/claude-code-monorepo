...
function NovaEmpresa() {
  ...
  const [created, setCreated] = useState<{ email: string; username: string | null; password: string | null } | null>(null);

  useEffect(() => {
    if (!BILLING_ENABLED) return; // sem cobrança não há planos a carregar
    void (async () => {
      try {
    ...
    try {
      const r = await create({
        data: {
          ...
          planId: BILLING_ENABLED ? (planId || null) : null,
          trialDays: BILLING_ENABLED ? trialDays : 0,
          password: password || null,
        },
      });
      toast.success(BILLING_ENABLED ? "Empresa criada e liberada com trial" : "Empresa criada e acesso liberado");
      setCreated({ email: ownerEmail, username: r.username ?? null, password: r.tempPassword ?? (password || null) });
      setNome(""); setOwnerEmail(""); setUsername(""); setPassword("");
    ...
    <div className="space-y-5 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {BILLING_ENABLED
            ? "Cria a empresa, o usuário responsável e libera o plano com período de teste. Ao fim do trial o cliente precisa cadastrar o cartão para continuar."
            : "Cria a empresa e o usuário responsável com acesso liberado. Informe nome, e-mail e senha — o cliente já entra direto, sem e-mail de confirmação."}
        </p>
      </div>
      ...
      <Card className="p-5">
        <form onSubmit={submit} className="space-y-5">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="ex.: padariadobairro"
                autoComplete="off"
              />
              <p className="text-[11px] text-muted-foreground mt-1">Login curto, além do e-mail. Em branco = gerado do e-mail.</p>
            </div>
            <div>
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={busy || (BILLING_ENABLED && !planId)}>
        ...
        {created && (
          <div className="mt-5 rounded-xl border bg-emerald-500/10 p-4 space-y-2">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              ...
              {created.username && <Field label="Usuário" value={created.username} />}
