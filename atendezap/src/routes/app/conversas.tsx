...
import { Hand, MessageSquareText, Send, Sparkles, User, Search, Bot, ExternalLink, ArrowLeft } from "lucide-react";
...
function ConversasPage() {
  ...
  return (
    <div className="space-y-4">
      <header className={`${active ? "hidden md:flex" : "flex"} items-end justify-between gap-3 flex-wrap`}>
        <div>
      ...
      <div className={`grid md:grid-cols-[320px_1fr] xl:grid-cols-[320px_1fr_300px] border border-[color:var(--hairline)] rounded-2xl overflow-hidden ${active ? "h-[calc(100dvh-190px)]" : "h-[calc(100dvh-230px)]"} md:h-[calc(100vh-200px)] min-h-[460px] md:min-h-[500px] bg-[color:var(--panel)]`}>
        {/* LISTA — no mobile, esconde quando há conversa aberta */}
        <aside className={`border-r border-[color:var(--hairline)] ${active ? "hidden md:flex" : "flex"} flex-col min-h-0 bg-[color:var(--panel)]`}>
        ...
        {/* THREAD — no mobile, só aparece quando há conversa aberta */}
        <section className={`${active ? "flex" : "hidden md:flex"} flex-col min-h-0 bg-[color:var(--panel-2)]`}>
          {!active ? (
            ...
            <>
              <header className="flex items-center gap-3 px-4 py-3 border-b border-[color:var(--hairline)] bg-[color:var(--panel)]">
                <button
                  ...
                  className="md:hidden -ml-1.5 size-9 grid place-items-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-[color:var(--panel-2)] shrink-0"
