...
/** Garante que o username não colide com outro já usado (varre metadata do Auth). */
async function ensureUniqueUsername(supabaseAdmin: any, desired: string): Promise<string> {
  ...
  for (let i = 1; i < 100 && taken.has(candidate); i++) candidate = `${desired}${i}`;
  return candidate;
}
...
export const createCompanyWithOwner = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: {
    nome: string; ownerEmail: string; username?: string | null;
    ...
    let username = d.username ? String(d.username).trim().toLowerCase() : "";
    if (username) {
      if (!/^[a-z0-9._-]{3,30}$/.test(username)) throw new Error("Usuário: use 3-30 caracteres (letras, números, . _ -)");
      if (username.includes("@")) throw new Error("Usuário não pode conter @");
    }
    return {
      nome, ownerEmail,
  ...
  .handler(async ({ context, data }) => {
    ...
    // username: usa o informado ou deriva do e-mail; garante unicidade
    const desiredUsername = data.username || data.ownerEmail.split("@")[0].toLowerCase();
    const username = await ensureUniqueUsername(supabaseAdmin, desiredUsername);
    ...
    if (!ownerId) {
      ...
      // Owner já existe: garante username no metadata e reseta senha se informada
      await supabaseAdmin.auth.admin.updateUserById(ownerId, {
        user_metadata: { username },
        ...(data.password ? { password: data.password, email_confirm: true } : {}),
      });
      if (data.password) tempPassword = data.password;
    }

    // Grava username em profiles (coluna pode não existir ainda — tenta e ignora)
    try {
      await supabaseAdmin.from("profiles").upsert(
        { user_id: ownerId, email: data.ownerEmail, username } as any,
        { onConflict: "user_id" },
      );
    } catch {
      await supabaseAdmin.from("profiles").upsert({ user_id: ownerId, email: data.ownerEmail }, { onConflict: "user_id" });
    }
    ...
    return { ok: true, companyId: comp.id, tempPassword, username };
  });
...
/** Edita dados da empresa e do responsável (nome, e-mail, telefone) — para repaginar a conta demo antes de reuniões. */
export const updateCompanyAndOwner = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: {
    companyId: string;
    companyName?: string; telefone?: string; ownerName?: string; ownerEmail?: string;
  }) => {
    const companyId = String(d.companyId || "");
    if (!companyId) throw new Error("Empresa inválida");
    const ownerEmail = d.ownerEmail ? String(d.ownerEmail).trim().toLowerCase() : undefined;
    if (ownerEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(ownerEmail)) throw new Error("E-mail inválido");
    return {
      companyId,
      companyName: d.companyName ? String(d.companyName).trim() : undefined,
      telefone: d.telefone !== undefined ? String(d.telefone).trim() : undefined,
      ownerName: d.ownerName !== undefined ? String(d.ownerName).trim() : undefined,
      ownerEmail,
    };
  })
  .handler(async ({ context, data }) => {
    ...
    if (data.companyName) { companyPatch.nome = data.companyName; companyPatch.nome_fantasia = data.companyName; }
    if (data.telefone !== undefined) companyPatch.telefone = data.telefone || null;
    if (data.ownerEmail) companyPatch.email_corporativo = data.ownerEmail;
    if (Object.keys(companyPatch).length) {
      const { error } = await supabaseAdmin.from("company").update(companyPatch).eq("id", data.companyId);
      if (error) throw new Error(error.message);
    }
    // Agente usa o nome da empresa
    if (data.companyName) {
      await supabaseAdmin.from("agent_config").update({ nome_empresa: data.companyName }).eq("company_id", data.companyId);
    }

    // Responsável (owner)
    const { data: cu } = await supabaseAdmin
      .from("company_user").select("user_id, role").eq("company_id", data.companyId).eq("ativo", true)
      .order("created_at", { ascending: true });
    const owner = (cu ?? []).find((r: any) => r.role === "owner") ?? (cu ?? [])[0];
    if (owner) {
      const profPatch: any = {};
      if (data.ownerName !== undefined) profPatch.nome = data.ownerName || null;
      if (data.ownerEmail) profPatch.email = data.ownerEmail;
      if (Object.keys(profPatch).length) {
        await supabaseAdmin.from("profiles").update(profPatch).eq("user_id", owner.user_id);
      }
      if (data.ownerEmail) {
        const { error: authErr } = await supabaseAdmin.auth.admin.updateUserById(owner.user_id, { email: data.ownerEmail, email_confirm: true });
        if (authErr) throw new Error(`E-mail: ${authErr.message}`);
      }
    }
