# LOOP.md — prompt canônico da sessão do gov-loop — PRÁXIS (Governança de Atendimento)

> Você é UMA sessão DESCARTÁVEL do loop de construção do épico de governança de
> atendimento do PRÁXIS (spec-mãe: `docs/specs/13-spec-governanca-atendimento.md`;
...
> (`loop/loop.config.json → main_checkout`, hoje `/Users/rafaelmelgaco/PRÁXIS`).
...
- **Briefing ao subagente** (padrão dispatch — contexto completo, uma tarefa):
  ...
  toca); e as **restrições de 1ª ordem do PRÁXIS** (o CLAUDE.md do repo é a
