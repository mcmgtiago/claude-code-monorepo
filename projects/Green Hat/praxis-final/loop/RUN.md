...
**Onde o loop RODA: no checkout principal — `/Users/rafaelmelgaco/PRÁXIS`
(`loop/loop.config.json → main_checkout`) — DEPOIS que a branch `gov/setup` for
mergeada em `main`.** O worktree `PRÁXIS-gov` existe só para montar a
...
- `plan/features.json` populado (26 features G1-01..G6-04); `plan/progress.md`
  ...
  ```json
  {
    ...
    "main_checkout": "/Users/rafaelmelgaco/PRÁXIS",
    "parallel_ui_lane": false,
    "smoke": "pnpm typecheck && pnpm lint && pnpm test:unit"
  }
  ```
  ("hoje" do teto diário é no fuso America/Sao_Paulo.)
- Slash command (modo interativo): `.claude/commands/praxis-gov-loop.md`.
  **`/praxis-gov-loop`, não `/loop`** — `/loop` colide com a skill built-in
...
```bash
cd /Users/rafaelmelgaco/PRÁXIS && claude
> /praxis-gov-loop
```
Uma sessão = uma feature. Quer outra feature, abra OUTRA sessão (`claude` de novo
ou `/clear` antes de `/praxis-gov-loop`). Nunca encadeie duas features na mesma
...
#!/usr/bin/env bash
set -u
REPO="/Users/rafaelmelgaco/PRÁXIS"; cd "$REPO" || exit 1
...
LOCKDIR="/tmp/praxis-gov-loop.lock.d"
...
`~/Library/LaunchAgents/com.praxis.gov-loop.plist` com `ProgramArguments`
`/bin/bash -lc '/Users/rafaelmelgaco/PRÁXIS/loop/run-session.sh'` e
