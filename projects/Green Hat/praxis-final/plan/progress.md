# Progress — gov-loop (PRÁXIS · Governança de Atendimento)
...
- Próximo: dono revisa `gov/setup`, mergeia em `main`, e o loop abre G1
  (`/praxis-gov-loop`). G1-06 é human_input — as 5 decisões de produto.
...
- Handoff: próximas sessões (G2-02+) delegadas ao terminal Arquiteto e Executor
  (cooperação A2A retomada pelo dono); Maestro-PRÁXIS vira watchdog.
...
- 17:11: Terminal B criou loop/STOP alegando git clean ~17h apagando trabalho
  ...
  no worktree ../PRÁXIS-vendaval. Principal chegou a ser trocado pra
