...
**Status:** Adotada integralmente como linha de base arquitetural do PRÁXIS (decisão registrada em memória do projeto).

Esse documento extrai apenas as decisões e padrões da referência que o PRÁXIS herda. Para citações literais, schema SQL completo e edge cases detalhados, consultar a fonte original.
...
## 11. Gaps a desenhar do PRÁXIS (não cobertos pela referência)
...
5. **Super-admin de plataforma** — coluna `is_platform_admin` ou tabela separada; helper RLS retorna TRUE para essa role; UI separada (`admin.praxis.com`)
