...
- Fica na **raiz do projeto**: `/seu-caminho/PRÁXIS/.env.local`.
...
2. No dashboard, clique **New project**.
   - **Name:** `praxis-dev` (ou o que quiser).
...
3. Clique **Create API Key** → nome `praxis-dev` → copie a chave.
...
3. **Settings → API Keys → Create Key** → nome `praxis-dev` → copie.
...
3. **API Keys → Create new secret key** → nome `praxis-dev-embeddings` → copie.
...
- Confira logs do container: `docker logs praxis-waha`.
...
Bem-vindo ao PRÁXIS. 🛠️

---

> **Achou um erro neste guia?** Abra uma [issue](https://github.com/melgarafael/PRÁXIS/issues) ou mande um PR — esse doc vive da contribuição da comunidade.
