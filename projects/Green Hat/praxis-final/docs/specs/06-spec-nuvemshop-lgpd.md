...
|---|---|---|
| App embedded vs External | **External** | Mais flexibilidade de UI custom (admin PRÁXIS tem UX própria); evita iframe sandbox; consent renderizado no domínio Nuvemshop é suficiente |
...
export class NuvemshopAdapter implements EcommercePlatformAdapter {
  ...
  private async request<T>(ctx: AdapterContext, path: string, init: RequestInit & { idempotencyKey?: string } = {}): Promise<{ data: T; rateLimit: FetchPage<T>['rateLimit'] }> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'PRÁXIS/1.0 (rafael@maudibrasil.com.br)',
...
- UI custom no admin PRÁXIS (sem iframe Nuvemshop sandbox)
- Permite callback em domínio próprio com cookie de sessão PRÁXIS
...
## 8. Mapping Nuvemshop → PRÁXIS

### 8.1 `customer.email/phone/identification` → `contacts`

| Campo Nuvemshop | Campo PRÁXIS | Notas |
|---|---|---|
