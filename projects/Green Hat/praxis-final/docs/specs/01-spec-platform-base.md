...
> Spec foundational do PRÁXIS. Define schema SQL completo, RLS policies, fluxos de auth, contratos de API REST `/api/v1/`, endpoints LGPD e onboarding de tenant. Toda spec posterior (Customer 360, WhatsApp, Pipeline, IA, Nuvemshop) **depende** desta. Divergências exigem ADR explícito.
...
comment on table public.organizations is 'Tenants do PRÁXIS. Cada linha = 1 e-commerce cliente.';
...
Tenant criado:
  ...
  Slug:  loja-exemplo
  URL:   https://loja-exemplo.praxis.com (DNS pending)
  Admin invite: https://app.praxis.com/invite/<jwt-1h>
...
Wizard em `https://admin.praxis.com/tenants/new` com mesma sequência:
...
export const log = pino({
  ...
  base: { service: 'praxis-api', env: process.env.NODE_ENV },
  redact: {
