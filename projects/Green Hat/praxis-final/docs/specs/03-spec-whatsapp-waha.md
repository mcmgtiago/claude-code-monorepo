...
A `WAHA_API_KEY` do servidor é o **hash SHA512 hex (lowercase) do plaintext**. O backend PRÁXIS guarda **só** o plaintext em Vercel Encrypted Env Var; nunca a hash duplicada. Geração:
...
WAHA_DASHBOARD_USERNAME=admin_praxis
WAHA_DASHBOARD_PASSWORD=<senha forte gerada>
WAHA_S3_REGION=auto
WAHA_S3_BUCKET=praxis-waha-media
...
WAHA_BASE_URL=https://waha.praxis.internal
WAHA_WEBHOOK_PUBLIC_BASE_URL=https://api.praxis.com
...
server {
  ...
  server_name waha.praxis.internal;

  ssl_certificate     /etc/letsencrypt/live/waha.praxis.internal/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/waha.praxis.internal/privkey.pem;
...
1. Verificar logs WAHA (`docker logs praxis-waha --tail 500`).
...
- Domínio gerado pela Railway (`waha-praxis.up.railway.app`); usado em `WAHA_BASE_URL` (Vercel).
- Custo: $5-10/mês.
...
- Instalação:
  ...
  certbot --nginx -d waha.praxis.internal
  cd /opt/praxis-waha && docker compose up -d
  ```
