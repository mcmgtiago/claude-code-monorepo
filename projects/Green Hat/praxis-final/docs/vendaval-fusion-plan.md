# Fusão Vendaval → PRÁXIS — briefing de execução (Terminal B)
...
**Um repositório só: o PRÁXIS.** Fusão total do back-end do Vendaval (bom, mas nunca ligado à
realidade) dentro do PRÁXIS (Next.js + TS + Supabase + WAHA + AI SDK — CRM real com chat ao vivo,
...
PRÁXIS como borda — o contrato exato está em `~/vendaval/docs/specs/edge-contract.md` (referencia
arquivos e linhas reais do Práxis). Como agora é UM repo, a maquinaria de "dois sistemas" colapsa em
chamadas internas.

**Encaixes verificados no código real do Práxis:**
...
- Entrada: WAHA → `event_log` (RPC `emit_event`). O Práxis tem um dispatcher nativo (`dispatchAgents`).
...
- Modelo de dados do Práxis é o CANÔNICO: `organizations`/`contacts`/`conversations`/`messages`/
...
→ resposta aparece no CHAT do Práxis. *Prova:* você manda um WhatsApp de verdade a um número conectado e VÊ o
...
**Fase 2 — continuidade + config real.** Memória/RAG/follow-up no turno (usando histórico real do Práxis) +
...
- Do Práxis: siga o `CLAUDE.md`/`AGENTS.md`/MANIFEST do repo (migrations, RLS, LGPD nativa, multi-tenant).
