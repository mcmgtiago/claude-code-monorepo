...
- **W9 — Billing**: `/app/settings/billing` static placeholder card pointing to `suporte@praxis.app`.
...
Placeholder explícito pra não deixar item quebrado na sidebar de settings. Mostra mensagem "Em breve" + benefícios do plano + link `mailto:contato@praxis.com.br?subject=Billing` ou link pro WhatsApp da Práxis. Nenhuma lógica de billing real.
...
And botão "Falar com a Práxis" abre mailto
```
