...
describe("executeCallWebhook", () => {
  ...
  it("sucesso: envia envelope correto, sem assinatura, sem organization_id", async () => {
    ...
    expect(received!.headers["x-praxis-event"]).toBe("lead.created");
    expect(received!.headers["x-praxis-signature"]).toBeUndefined();
  ...
  it("com secret: header de assinatura HMAC-sha256 do body", async () => {
    ...
    expect(received!.headers["x-praxis-signature"]).toBe(expectedSig);
