...
PRÁXIS é um sistema de IA para consultórios médicos de especialidades delicadas (urologia, proctologia, andrologia, coloproctologia). Remove a barreira emocional entre paciente e consultório via WhatsApp e chat. Agentes com RAG por tenant atendem, qualificam e movem o funil junto com humanos; CRM + Agenda + Prontuário exposto via MCP. Arquitetura multi-tenant com RLS desde o dia 1; LGPD nativa. Baseado no DeskcommCRM (fork), enriquecido com features de Odontocontrol (agenda, pacientes, procedimentos) e BeautyFlow (agendamento público, pacotes). IA via Avello Gateway (Anthropic). Posicionamento completo: `PLAN.md`.
...
**O PRÁXIS é distribuído open-source: a experiência de quem instala numa VPS É o produto.** Toda feature nova (ou fix de comportamento visível) DEVE ser provada como um **usuário leigo a usaria de verdade** — pelo frontend, num ambiente que imita a instalação fresca — antes de "pronto". Não é opcional; é critério de aceite de toda sessão que toca UI ou fluxo de usuário.
