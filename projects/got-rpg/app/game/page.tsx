...
export default function GamePage() {
  ...
  const [cfgTier, setCfgTier] = useState<'plebeu' | 'cavaleiro' | 'nobre_menor' | 'rei'>('plebeu');
  const [cfgPower, setCfgPower] = useState<'magia' | 'mundano'>('magia');
  ...
  const [cfgAttrs, setCfgAttrs] = useState({ forca: 10, destreza: 10, resistencia: 10, astucia: 10, comando: 10, seducao: 10, percepcao: 10, vontade: 10 });

  const TIER_BUDGETS: Record<string, number> = { plebeu: 20, cavaleiro: 30, nobre_menor: 40, nobre_maior: 55, rei: 80 };
  ...
  const ATTR_LABELS: Record<string, string> = { forca: 'FOR', destreza: 'DES', resistencia: 'RES', astucia: 'AST', comando: 'CMD', percepcao: 'PER', vontade: 'VON', seducao: 'SED' };

  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);
  useEffect(() => { if (cfgTier === 'rei') setCfgAttrs({ forca: 18, destreza: 18, resistencia: 18, astucia: 18, comando: 18, seducao: 18, percepcao: 18, vontade: 18 }); else setCfgAttrs({ forca: 10, destreza: 10, resistencia: 10, astucia: 10, comando: 10, seducao: 10, percepcao: 10, vontade: 10 }); }, [cfgTier]);
  ...
  const startGame = () => {
    ...
    const tierLabel = cfgTier === 'rei' ? 'REI' : cfgTier === 'nobre_menor' ? 'NOBRE' : cfgTier === 'cavaleiro' ? 'CAVALEIRO' : 'PLEBEU';
    const powerStr = cfgPower === 'mundano' ? 'MUNDANO (sem magia)' : cfgPowerType;

    const scenarioInstr = selectedScenario
      ? `CENÁRIO DE INÍCIO: "${selectedScenario.title}" — ${selectedScenario.setup}`
      : 'Escolha um cenário de início ALEATÓRIO e criativo em Westeros.';

    const growthInstr = (cfgTier === 'rei' || cfgTier === 'nobre_menor')
      ? 'PODER: Personagem já começa PODEROSO — grande lorde/rei com exército, terras, influência. Enfrenta ameaças políticas de alto nível.'
      : 'PODER: Personagem começa HUMILDE (plebeu/cavaleiro) mas com ASCENSÃO INSANAMENTE RÁPIDA. Conceda XP MUITO generoso (60-150 por feito, 300+ por marcos políticos). Terras, títulos e aliados vêm rápido. De plebeu a lorde em tempo recorde.';

    const modeInstr = gameMode === 'adult'
      ? `MODO +18 ATIVO. Westeros é sensual e cruel — sexo é arma política. ${scenarioInstr} Descreva com TENSÃO SEXUAL, corpo do NPC, provocação. NPCs sedutores. PARE antes do explícito, deixe o jogador decidir.`
      : `MODO AVENTURA. ${scenarioInstr} Foco em intriga política, guerra, sobrevivência e lore de Westeros. Conteúdo +18 permitido se o jogador iniciar.`;

    const instruction = `INICIAR NOVO JOGO — GAME OF THRONES.
${modeInstr}
${growthInstr}
Personagem: "${cfgName}"${cfgAge ? `, ${cfgAge} anos` : ''}, ${tierLabel}, ${powerStr}${cfgPersonality ? `, personalidade: ${cfgPersonality}` : ''}${cfgAppearance ? `, aparência: ${cfgAppearance}` : ''}
Chame create_character com: name="${cfgName}", house="${cfgPersonality || 'plebeu'}", era="guerra_cinco_reis", tier="${cfgTier}", title="${tierLabel === 'PLEBEU' ? 'plebeu' : tierLabel === 'CAVALEIRO' ? 'cavaleiro' : tierLabel === 'NOBRE' ? 'lorde' : 'rei/rainha'}", ${cfgPower === 'magia' ? `magic_type="${cfgPowerType || 'warg'}", magic_description="${cfgPowerType}",` : ''} background="Nascido em Westeros"${cfgAge ? `, age=${cfgAge}` : ''}, personality="${cfgPersonality || ''}", appearance="${cfgAppearance || ''}", attributes=${attrsObj}.

DEPOIS de criar, narre a cena de início conforme o cenário escolhido. MÁXIMO 2 parágrafos. Detalhe o ambiente/NPCs presentes. NÃO tome ações pelo jogador — termine esperando a decisão dele.`;
  ...
  const generatePortrait = async (appearance: string, name: string) => {
    try {
      const res = await fetch('/api/game/image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'character', data: { name: name || 'westeros character', appearance } }) });
  ...
  // Som de notificação quando resposta chega (Web Audio API, sem arquivo)
  ...
  const [rndMutant, setRndMutant] = useState<'magia' | 'mundano'>('magia');
  const [rndTier, setRndTier] = useState<'plebeu' | 'cavaleiro' | 'nobre_menor' | 'rei'>('cavaleiro');
  ...
  const [rndPhase, setRndPhase] = useState<'config' | 'preview' | 'photos' | 'scenarios'>('config');
  ...
  if (screen === 'menu') return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-black p-6">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight text-white mb-2">GAME OF THRONES</h1>
        <p className="text-sm text-gray-600">RPG Narrativo • Westeros</p>
      </div>
  ...
  if (screen === 'random') {
    const generateRandomChar = async () => {
      ...
      try {
        const res = await fetch('/api/game/generate', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'character', sex: rndSex, house: 'aleatória', era: 'Guerra dos Cinco Reis', tier: rndTier, magic: rndMutant === 'magia' ? 'aleatória' : 'nenhuma' }),
        });
        const data = await res.json();
    ...
    const startWithRandom = () => {
      const sid = Math.random().toString(36).slice(2) + Date.now().toString(36);
      setSessionId(sid); setMessages([]); setGameState(null); setCrossroad(null); setScreen('game');
      const budgets: Record<string, number> = { plebeu: 20, cavaleiro: 30, nobre_menor: 40, rei: 80 };
      const b = budgets[rndTier];
      const attrs = { forca: 8, destreza: 8, resistencia: 8, astucia: 8, comando: 8, seducao: 8, percepcao: 8, vontade: 8 };
      let pts = b;
      ...
      const tierLabel = rndTier === 'rei' ? 'REI' : rndTier === 'nobre_menor' ? 'NOBRE' : rndTier === 'cavaleiro' ? 'CAVALEIRO' : 'PLEBEU';

      const scenarioInstr = selectedScenario
        ? `CENÁRIO: "${selectedScenario.title}" — ${selectedScenario.setup}`
        : 'Cenário ALEATÓRIO em Westeros.';

      const growthInstr = (rndTier === 'rei' || rndTier === 'nobre_menor')
        ? 'Já começa PODEROSO.'
        : 'Começa fraco com ASCENSÃO INSANAMENTE RÁPIDA (XP 2-3x, skills frequentes).';

      const modeInstr = gameMode === 'adult'
        ? `MODO +18. ${scenarioInstr} Tensão sexual, NPCs sedutores. PARE antes do explícito.`
        : `MODO AVENTURA. ${scenarioInstr} Intriga, guerra, sobrevivência.`;

      const instruction = `INICIAR NOVO JOGO — GAME OF THRONES.
${modeInstr} ${growthInstr}
Personagem: "${g.name}", ${g.age} anos, ${tierLabel}. Magia: ${g.magic || g.power || 'nenhuma'}. Personalidade: ${g.personality}. Aparência: ${g.appearance}. Background: ${g.background}.
Chame create_character com: name="${g.name}", age=${g.age}, house="${g.house || 'nenhuma'}", era="guerra_cinco_reis", tier="${rndTier}", ${rndMutant === 'magia' ? `magic_type="${g.magic || g.power}", magic_description="${g.magic || g.power}",` : ''} personality="${g.personality}", appearance="${g.appearance}", background="${g.background}", attributes=${attrsObj}.
DEPOIS narre conforme o cenário. MÁXIMO 2 parágrafos. NÃO tome ações pelo jogador.`;
        ...
        <div className="max-w-md mx-auto">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setRndMutant('magia')} className={`py-3 border rounded-lg text-sm active:scale-95 ${rndMutant === 'magia' ? 'border-purple-500 text-white bg-white/5' : 'border-gray-800 text-gray-500'}`}>⚔️ Com Magia (Warg, R'hllor...)</button>
                    <button onClick={() => setRndMutant('mundano')} className={`py-3 border rounded-lg text-sm active:scale-95 ${rndMutant === 'mundano' ? 'border-gray-400 text-white bg-white/5' : 'border-gray-800 text-gray-500'}`}>🛡️ Sem Magia</button>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] text-gray-600 uppercase mb-2">Classificação</p>
                  <div className="grid grid-cols-2 gap-2">
                    {(['plebeu', 'cavaleiro', 'nobre_menor', 'rei'] as const).map(t => (
                      <button key={t} onClick={() => setRndTier(t)} className={`py-2 border rounded-lg text-xs active:scale-95 ${rndTier === t ? 'border-white text-white bg-white/5' : 'border-gray-800 text-gray-600'}`}>
                        {t === 'rei' ? 'Rei' : t === 'nobre_menor' ? 'Nobre' : t === 'cavaleiro' ? 'Cavaleiro' : 'Plebeu'}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {rndPhase === 'preview' && rndGenerated && (
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setScenarioOptions(getRandomScenarios(gameMode, 5)); setSelectedScenario(null); setRndPhase('scenarios'); }} className="flex-1 py-3 bg-white text-black font-semibold rounded-lg active:scale-95">
                  Continuar →
                </button>
              </div>
            </div>
          ...
          {rndPhase === 'photos' && rndGenerated && (
            <div>
              ...
              <button onClick={() => { setScenarioOptions(getRandomScenarios(gameMode, 5)); setSelectedScenario(null); setRndPhase('scenarios'); }} className="w-full py-4 bg-white text-black font-bold rounded-lg active:scale-95">
                Continuar →
              </button>
            </div>
          )}
          ...
          {/* Fase Scenarios */}
          {rndPhase === 'scenarios' && rndGenerated && (
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Como começa?</h2>
              ...
              <div className="space-y-2 mb-4">
                {scenarioOptions.map(sc => (
                  <button key={sc.id} onClick={() => setSelectedScenario(sc)}
                    className={`w-full text-left p-3 border rounded-lg active:scale-95 ${selectedScenario?.id === sc.id ? 'border-amber-500 bg-white/5' : 'border-gray-800 hover:border-gray-600'}`}>
              ...
              <div className="flex gap-2 mb-3">
                <button onClick={() => setScenarioOptions(getRandomScenarios(gameMode, 5))} className="flex-1 py-2 border border-gray-800 rounded-lg text-xs text-gray-400 active:scale-95">🔄 Outros</button>
                <button onClick={() => setSelectedScenario(null)} className={`flex-1 py-2 border rounded-lg text-xs active:scale-95 ${!selectedScenario ? 'border-white text-white' : 'border-gray-800 text-gray-500'}`}>🎲 Surpresa</button>
              </div>
  ...
  if (screen === 'config') {
    const TIERS = { plebeu: { label: 'Plebeu', desc: 'Camponês', color: 'border-gray-600' }, cavaleiro: { label: 'Cavaleiro', desc: 'Guerreiro', color: 'border-blue-500' }, nobre_menor: { label: 'Nobre', desc: 'Lorde', color: 'border-purple-500' }, rei: { label: 'Rei', desc: 'Grande Lorde', color: 'border-amber-400' } };
    ...
    const generatePhotos = async () => {
      ...
      const desc = cfgAppearance || `${cfgName}, ${cfgAge ? cfgAge + ' anos, ' : ''}${cfgPersonality ? cfgPersonality + ', ' : ''}${cfgPower === 'magia' ? 'com magia: ' + cfgPowerType : 'mundano'}`;
      try {
        const results = await Promise.all([1,2,3,4].map(async (i) => {
      ...
      <div className="min-h-[100dvh] bg-black p-4 overflow-y-auto">
        <div className="max-w-md mx-auto">
          ...
          <div className="flex gap-1 mb-6">
            {[1,2,3,4,5,6].map(s => <div key={s} className={`flex-1 h-0.5 rounded-full ${s <= cfgStep ? 'bg-white' : 'bg-gray-800'}`} />)}
          </div>
          ...
          {/* Step 3: Com Magia ou Mundano */}
          {cfgStep === 3 && (
            <div>
              <h2 className="text-xl font-bold text-white mb-1">{cfgName} é magia?</h2>
              <p className="text-xs text-gray-600 mb-6">Define se começa com poderes</p>
              <div className="space-y-3">
                <button onClick={() => { setCfgPower('magia'); setCfgStep(4); }}
                  className="w-full p-4 border border-gray-800 hover:border-purple-500 rounded-lg text-left active:scale-95">
                  <p className="text-white font-semibold">⚔️ Com Magia (Warg, R'hllor...)</p>
                  <p className="text-xs text-gray-500 mt-1">Warg, Vidente, R'hllor, Sangue Valyrio, Sem-Rosto...</p>
                </button>
                <button onClick={() => { setCfgPower('mundano'); setCfgPowerType('nenhum'); setCfgStep(5); }}
                  className="w-full p-4 border border-gray-800 hover:border-gray-400 rounded-lg text-left active:scale-95">
                  <p className="text-white font-semibold">🛡️ Sem Magia</p>
                  <p className="text-xs text-gray-500 mt-1">Soldado, político, mercenário. Sem magia.</p>
                </button>
              </div>
            </div>
          )}
          ...
          {/* Step 6: Aparência + Fotos (ÚLTIMO) */}
          {cfgStep === 6 && (
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Como {cfgName} se parece?</h2>
              <p className="text-xs text-gray-600 mb-4">Descreva rosto, corpo, roupas — depois gere retratos</p>
              ...
              <textarea value={cfgAppearance} onChange={e => setCfgAppearance(e.target.value)}
                ...
                className="w-full px-3 py-3 bg-black border border-gray-800 rounded-lg text-white placeholder-gray-700 focus:border-white text-sm h-28 resize-none mb-3" />

              <button onClick={generatePhotos} disabled={cfgGeneratingPhotos}
                className="w-full py-2.5 border border-gray-800 rounded-lg text-sm text-gray-400 hover:text-white hover:border-white active:scale-95 disabled:opacity-50 mb-4">
                {cfgGeneratingPhotos ? '⏳ Gerando 4 retratos...' : '📷 Gerar 4 retratos'}
              </button>

              {cfgPhotos.length > 0 && (
                <div className="grid grid-cols-2 gap-2 mb-4">
                  {cfgPhotos.map((url, i) => (
                    <button key={i} onClick={() => { setCfgSelectedPhoto(i); setPortrait(url); }}
                      className={`rounded-lg overflow-hidden border-2 active:scale-95 ${cfgSelectedPhoto === i ? 'border-white' : 'border-gray-800 hover:border-gray-600'}`}>
                      <img src={url} alt={`Opção ${i+1}`} className="w-full aspect-square object-cover" />
                    </button>
                  ))}
                  <button onClick={generatePhotos} disabled={cfgGeneratingPhotos}
                    className="col-span-2 py-2 text-[10px] text-gray-600 hover:text-white">🔄 Gerar outras fotos</button>
                </div>
              )}
          ...
          {/* Step 7: Cenário de Início */}
          {cfgStep === 7 && (
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Como sua história começa?</h2>
              <p className="text-xs text-gray-600 mb-4">Escolha um cenário — ou deixe aleatório</p>
              <div className="space-y-2 mb-4">
                {scenarioOptions.map(sc => (
                  <button key={sc.id} onClick={() => setSelectedScenario(sc)}
                    className={`w-full text-left p-3 border rounded-lg active:scale-95 transition-all ${selectedScenario?.id === sc.id ? 'border-amber-500 bg-white/5' : 'border-gray-800 hover:border-gray-600'}`}>
                    <p className="text-white text-sm font-semibold">{sc.emoji} {sc.title}</p>
                    <p className="text-gray-500 text-[11px] mt-0.5">{sc.setup.slice(0, 90)}...</p>
                  </button>
                ))}
              ...
              <div className="flex gap-2 mb-3">
                <button onClick={() => setScenarioOptions(getRandomScenarios(gameMode, 5))} className="flex-1 py-2 border border-gray-800 rounded-lg text-xs text-gray-400 hover:text-white active:scale-95">🔄 Outros</button>
                <button onClick={() => setSelectedScenario(null)} className={`flex-1 py-2 border rounded-lg text-xs active:scale-95 ${!selectedScenario ? 'border-white text-white' : 'border-gray-800 text-gray-500'}`}>🎲 Surpresa</button>
              </div>
            ...
            <input
              ...
              onKeyDown={async e => {
                if (e.key === 'Enter' && imageGenPrompt.trim() && !imageGenLoading) {
                  ...
                  try {
                    const res = await fetch('/api/game/image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'scene', data: { prompt: imageGenPrompt + '. Medieval dark fantasy Game of Thrones style, cinematic, detailed, atmospheric. No text.' } }) });
                    const data = await res.json();
                    if (data.url) {
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setShowInventory(false)}>
          <div className="bg-black border border-gray-800 rounded-xl w-full max-w-md max-h-[80vh] overflow-y-auto p-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-1">
                {['nenhuma', 'aprendiz', 'praticante', 'mestre', 'lendário'].map(s => (
