...
export default function GamePage() {
  ...
  const [cfgTier, setCfgTier] = useState<'novato' | 'veterano' | 'elite' | 'capitao'>('novato');
  const [cfgPower, setCfgPower] = useState<'shinigami' | 'humano_puro'>('shinigami');
  ...
  const [cfgAttrs, setCfgAttrs] = useState({ reiatsu: 10, zanjutsu: 10, hakuda: 10, hoho: 10, kido: 10, percepcao: 10, vontade: 10, presenca: 10 });

  const TIER_BUDGETS: Record<string, number> = { novato: 22, veterano: 34, elite: 48, capitao: 80 };
  ...
  useEffect(() => { if (cfgTier === 'capitao') setCfgAttrs({ reiatsu: 18, zanjutsu: 18, hakuda: 18, hoho: 18, kido: 18, percepcao: 18, vontade: 18, presenca: 18 }); else setCfgAttrs({ reiatsu: 10, zanjutsu: 10, hakuda: 10, hoho: 10, kido: 10, percepcao: 10, vontade: 10, presenca: 10 }); }, [cfgTier]);
  ...
  const startGame = () => {
    ...
    const tierLabel = cfgTier === 'capitao' ? 'REI' : cfgTier === 'elite' ? 'NOBRE' : cfgTier === 'veterano' ? 'CAVALEIRO' : 'PLEBEU';
    const powerStr = cfgPower === 'humano_puro' ? 'HUMANO PURO (sem poderes)' : cfgPowerType;

    const modeInstr = gameMode === 'adult'
      ? `MODO +18 ATIVO — INÍCIO ERÓTICO OBRIGATÓRIO.
O jogo COMEÇA com uma cena adulta/erótica/sexy. O transporte dimensional coloca o personagem numa situação sexual IMEDIATA. Possibilidades de início (escolha UMA aleatoriamente):
- Acorda nu/nua na cama de alguém atraente que está te tocando
- Cai no meio de um clube BDSM underground de shinigamis
- Materializa num quarto de hotel onde uma shinigami sedutora te esperava
- Acorda sendo amarrado/a por uma vilã que te capturou pra "brincar"
- Aparece num bordel de Madripoor, uma prostituta shinigami te achou desmaiado
- Cai num vestiário da Mansão X, alguém gostosa tá no banho e te vê
- Materializa num ritual erótico do Hellfire Club, rodeado de corpos
- Acorda no colo de uma shinigami dominadora que te "resgatou"
- Surge numa orgia de shinigamis num bunker secreto
- Aparece encostado numa parede num beco, alguém de joelhos na sua frente
IMPORTANTE: descreva a cena com TENSÃO SEXUAL, detalhes do corpo do NPC, provocação. O NPC FALA (dirty talk leve ou provocante). Mas PARE antes de ação explícita — deixe o jogador decidir se quer continuar ou sair da situação. Mundo sexualizado: NPCs atraentes, sedutores, provocantes.`
      : `MODO AVENTURA. Foco em combate, exploração, lore e história. Conteúdo +18 (sexo, violência gráfica) é PERMITIDO a qualquer momento se o jogador iniciar — não bloqueie. Só o INÍCIO é focado em história, não em sexo.`;
...
Chame create_character com: name="${cfgName}", race="${cfgPower === 'humano_puro' ? 'humano_puro_puro' : 'shinigami'}", tier="${cfgTier}", power_class="${cfgPower === 'humano_puro' ? 'nenhum' : cfgPowerType}", background="Transportado de outro universo"${cfgAge ? `, age=${cfgAge}` : ''}, personality="${cfgPersonality || ''}", attributes=${attrsObj}${cfgAppearance ? `, appearance="${cfgAppearance}"` : ''}.
...
5. MÁXIMO 2 parágrafos. NÃO tome ações pelo jogador.`}`;
  ...
  const [rndMutant, setRndMutant] = useState<'shinigami' | 'humano_puro'>('shinigami');
  const [rndTier, setRndTier] = useState<'novato' | 'veterano' | 'elite' | 'capitao'>('veterano');
  ...
  if (screen === 'menu') return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-black p-6">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight text-white mb-2">BLEACH</h1>
        <p className="text-sm text-gray-600">RPG Narrativo • Bleach Universe</p>
      </div>
  ...
  if (screen === 'random') {
    const generateRandomChar = async () => {
      ...
      try {
        const res = await fetch('/api/game/generate', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'character', sex: rndSex, house: 'aleatória', era: 'Guerra dos Cinco Capitãos', tier: rndTier, magic: rndMutant === 'shinigami' ? 'aleatória' : 'nenhuma' }),
        });
        const data = await res.json();
    ...
    const startWithRandom = () => {
      ...
      const tierLabel = rndTier === 'capitao' ? 'REI' : rndTier === 'elite' ? 'NOBRE' : rndTier === 'veterano' ? 'CAVALEIRO' : 'PLEBEU';
...
Chame create_character com: name="${g.name}", age=${g.age}, house="${g.house || 'aleatória'}", era="Guerra dos Cinco Capitãos", tier="${rndTier}", power_class="${rndMutant === 'humano_puro' ? 'nenhum' : g.power}", personality="${g.personality}", appearance="${g.appearance}", background="${g.background}", attributes=${attrsObj}.
DEPOIS: transporte breve (1 parágrafo) + acorda SOZINHO + 2-3 direções. MÁXIMO 2 parágrafos. NÃO tome ações.`;
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setRndMutant('shinigami')} className={`py-3 border rounded-lg text-sm active:scale-95 ${rndMutant === 'shinigami' ? 'border-purple-500 text-white bg-white/5' : 'border-gray-800 text-gray-500'}`}>⚡ Shinigami/Hollow/Quincy</button>
                    <button onClick={() => setRndMutant('humano_puro')} className={`py-3 border rounded-lg text-sm active:scale-95 ${rndMutant === 'humano_puro' ? 'border-gray-400 text-white bg-white/5' : 'border-gray-800 text-gray-500'}`}>🧑 Humano Comum</button>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] text-gray-600 uppercase mb-2">Classificação</p>
                  <div className="grid grid-cols-2 gap-2">
                    {(['novato', 'veterano', 'elite', 'capitao'] as const).map(t => (
                      <button key={t} onClick={() => setRndTier(t)} className={`py-2 border rounded-lg text-xs active:scale-95 ${rndTier === t ? 'border-white text-white bg-white/5' : 'border-gray-800 text-gray-600'}`}>
                        {t === 'capitao' ? 'Capitão' : t === 'elite' ? 'Elite' : t === 'veterano' ? 'Veterano' : 'Novato'}
                      </button>
                    ))}
                  </div>
                </div>
  ...
  if (screen === 'config') {
    const TIERS = { plebeu: { label: 'Novato', desc: 'Camponês', color: 'border-gray-600' }, cavaleiro: { label: 'Veterano', desc: 'Guerreiro', color: 'border-blue-500' }, nobre_menor: { label: 'Elite', desc: 'Lorde', color: 'border-purple-500' }, rei: { label: 'Capitão', desc: 'Grande Lorde', color: 'border-amber-400' } };
    ...
    const generatePhotos = async () => {
      ...
      const desc = cfgAppearance || `${cfgName}, ${cfgAge ? cfgAge + ' anos, ' : ''}${cfgPersonality ? cfgPersonality + ', ' : ''}${cfgPower === 'shinigami' ? 'shinigami com poder de ' + cfgPowerType : 'humano_puro'}`;
      try {
        const results = await Promise.all([1,2,3,4].map(async (i) => {
    ...
    return (
      <div className="min-h-[100dvh] bg-black p-4 overflow-y-auto">
        <div className="max-w-md mx-auto">
          ...
          {cfgStep === 3 && (
            <div>
              <h2 className="text-xl font-bold text-white mb-1">{cfgName} é shinigami?</h2>
              <p className="text-xs text-gray-600 mb-6">Define se começa com poderes</p>
              <div className="space-y-3">
                <button onClick={() => { setCfgPower('shinigami'); setCfgStep(4); }}
                  className="w-full p-4 border border-gray-800 hover:border-purple-500 rounded-lg text-left active:scale-95">
                  <p className="text-white font-semibold">⚡ Shinigami/Hollow/Quincy</p>
                  <p className="text-xs text-gray-500 mt-1">Zanpakutō, Cero, Heilig Pfeil...</p>
                </button>
                <button onClick={() => { setCfgPower('humano_puro'); setCfgPowerType('nenhum'); setCfgStep(5); }}
                  className="w-full p-4 border border-gray-800 hover:border-gray-400 rounded-lg text-left active:scale-95">
                  <p className="text-white font-semibold">🧑 Humano Comum</p>
                  <p className="text-xs text-gray-500 mt-1">Sem poderes espirituais. Pode despertar na história.</p>
                </button>
              </div>
            </div>
          )}
            ...
            <input
              ...
              onKeyDown={async e => {
                if (e.key === 'Enter' && imageGenPrompt.trim() && !imageGenLoading) {
                  ...
                  try {
                    const res = await fetch('/api/game/image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'scene', data: { prompt: imageGenPrompt + '. Bleach anime style, spiritual energy, Tite Kubo art, cinematic, detailed, atmospheric. No text.' } }) });
                    const data = await res.json();
                    if (data.url) {
